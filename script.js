// 时钟功能 --------------------------------------------------------------
window.usingApiTime = false; // 添加标记，指示是否正在使用API时间

window.startClock = function() {
	// 如果正在使用API时间，则不使用系统时间
	if (window.usingApiTime) {
	...
	// 添加平滑变化效果
	updateTimeWithAnimation(h, m, s);
	
	setTimeout(startClock, 500);
};

// 时间平滑变化动画函数
function updateTimeWithAnimation(h, m, s) {
    const timeElement = document.getElementById("current-time");
    
    // 添加淡入淡出效果
    timeElement.style.transition = "opacity 0.3s ease-in-out";
...
// 更新API巴士数据中的最大时间
window.updateBusDateTime = function(dateTime) {
    if (!dateTime) return;
    
    console.log("更新时间显示，接收到的时间对象:", dateTime);
    console.log("日期时间格式化:", dateTime.toLocaleString());
    
    // 设置标记，指示正在使用API时间
    window.usingApiTime = true;
    
    // 更新日期显示
    document.getElementById("current-date").innerText = getCurrentDate(dateTime);
    
    // 更新时间显示，使用动画效果
    const h = padWithZeroes(dateTime.getHours(), 2);
    const m = padWithZeroes(dateTime.getMinutes(), 2);
    const s = padWithZeroes(dateTime.getSeconds(), 2);
    
    // 使用动画更新时间
    updateTimeWithAnimation(h, m, s);
    
    // 添加API时间标识
    if (!document.getElementById("api-time-indicator")) {
        const timeBox = document.getElementById("current-time-box");
        const indicator = document.createElement("div");
        indicator.id = "api-time-indicator";
        indicator.style.fontSize = "12px";
        indicator.style.color = "#ffcc00"; // 黄色更醒目
        indicator.style.opacity = "1"; // 提高不透明度
        indicator.style.textAlign = "center";
        indicator.innerText = "API數據時間";
        timeBox.appendChild(indicator);
    }
};
